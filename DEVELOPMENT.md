...
| `scripts/sc-backup` | `sc_backup.py` | Uniblab | Preview, inspect status for, verify, restore-preview, or explicitly create SC operational backup snapshots. | Mixed: preview/status/verify/restore-preview are read-only; `--create` writes one backup directory. |
